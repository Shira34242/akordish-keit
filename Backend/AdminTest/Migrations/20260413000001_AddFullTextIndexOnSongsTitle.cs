...
namespace AkordishKeit.Migrations
{
    ...
    public partial class AddFullTextIndexOnSongsTitle : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(@"
                IF FULLTEXTSERVICEPROPERTY('IsFullTextInstalled') = 1
                   AND NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE name = 'SongCatalog')
                    CREATE FULLTEXT CATALOG SongCatalog AS DEFAULT;
            ", suppressTransaction: true);

            migrationBuilder.Sql(@"
                IF FULLTEXTSERVICEPROPERTY('IsFullTextInstalled') = 1
                   AND NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = OBJECT_ID('Songs'))
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"
                IF FULLTEXTSERVICEPROPERTY('IsFullTextInstalled') = 1
                   AND EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = OBJECT_ID('Songs'))
                    DROP FULLTEXT INDEX ON [Songs];
            ", suppressTransaction: true);

            migrationBuilder.Sql(@"
                IF FULLTEXTSERVICEPROPERTY('IsFullTextInstalled') = 1
                   AND EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE name = 'SongCatalog')
